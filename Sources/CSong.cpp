...
CSong::CSong(CMediaManager * mediaManager) :
...
CSong::CSong(const QString& fileName, CMediaManager * mediaManager) :
QObject            (mediaManager),
m_mediaManager     (mediaManager),
m_sound            (nullptr),
m_channel          (nullptr),
...
m_fileStatus       (true)
{
    ...
    m_id = getId(m_mediaManager, fileName);
...
void CSong::loadFromDatabase()
{
    ...
    if (!query.next())
    {
        m_mediaManager->logError(tr("invalid identifier (%1)").arg(m_id), __FUNCTION__, __FILE__, __LINE__);
        return;
    }
...
bool CSong::loadTags(bool readProperties)
{
    ...
    switch (m_properties.format)
    {
        ...
        case CSong::FormatMP3:
        {
            ...
            {
                m_fileStatus = false;
                m_mediaManager->logError(tr("can't read the MP3 file \"%1\"").arg(m_properties.fileName), __FUNCTION__, __FILE__, __LINE__);
                return false;
            }
        ...
        case CSong::FormatOGG:
        {
            ...
            {
                m_fileStatus = false;
                m_mediaManager->logError(tr("can't read the Ogg file \"%1\"").arg(m_properties.fileName), __FUNCTION__, __FILE__, __LINE__);
                return false;
            }
        ...
        case CSong::FormatFLAC:
        {
            ...
            {
                m_fileStatus = false;
                m_mediaManager->logError(tr("can't read the FLAC file \"%1\"").arg(m_properties.fileName), __FUNCTION__, __FILE__, __LINE__);
                return false;
            }
...
bool CSong::writeTags()
{
    ...
    switch (m_properties.format)
    {
        ...
        case CSong::FormatMP3:
        {
            ...
            {
                m_fileStatus = false;
                m_mediaManager->logError(tr("the file \"%1\" can't be opened").arg(m_properties.fileName), __FUNCTION__, __FILE__, __LINE__);
        ...
        case CSong::FormatOGG:
        {
            ...
            writeTags(file.tag(), m_infos, m_mediaManager->getLogFile("metadata"), m_properties.fileName);
...
bool CSong::moveFile()
{
    ...
    CLibraryFolder * libraryFolder = m_mediaManager->getLibraryFolder(m_mediaManager->getLibraryFolderId(m_properties.fileName));
    ...
    {
        m_mediaManager->logError(tr("can't rename the file \"%1\" to \"%2\"").arg(m_properties.fileName).arg(pathName), __FUNCTION__, __FILE__, __LINE__);
        return false;
    }
...
/*
            // Recherche de filtres spécifiques
            QRegExp rx("^{(lang|title):(\S+)}$");

            if (rx.pos(1) != 1 && rx.pos(2) != 1)
            {
                if (rx.cap(1) == "lang")
                {
                    if (rx.cap(2).size() == 2)
                    {
                        if (m_infos.language != getLanguageForISO2Code(rx.cap(2)))
                        {
                            return false;
                        }
                    }
                    else if (rx.cap(2).size() == 3)
                    {
                        if (m_infos.language != getLanguageForISO3Code(rx.cap(2)))
                        {
                            return false;
                        }
                    }
                ...
                else if (rx.cap(1) == "title")
                {
                    if (!m_infos.title.contains(rx.cap(2), Qt::CaseInsensitive))
                    {
                        return false;
                    }
                }
...
CSong * CSong::loadFromFile(CMediaManager * mediaManager, const QString& fileName)
{
    if (getId(mediaManager, fileName) >= 0)
    {
        mediaManager->logError(tr("the file \"%1\" is already in library").arg(fileName), __FUNCTION__, __FILE__, __LINE__);
        return nullptr;
    }
    ...
    // Chargement du son
    //res = mediaManager->getSoundSystem()->createStream(qPrintable(fileName), FMOD_LOOP_OFF | FMOD_HARDWARE | FMOD_2D, nullptr, &sound);
    res = mediaManager->getSoundSystem()->createStream(reinterpret_cast<const char *>(fileName.utf16()), FMOD_UNICODE | FMOD_LOOP_OFF | FMOD_HARDWARE | FMOD_2D, nullptr, &sound);

    if (res != FMOD_OK || !sound)
    {
        mediaManager->logError(tr("the file \"%1\" can't be opened with FMOD").arg(fileName), __FUNCTION__, __FILE__, __LINE__);
        return nullptr;
    }

    // Création du morceau
    CSong * song = new CSong(mediaManager);
    ...
    if (res != FMOD_OK)
    {
        mediaManager->logError(tr("can't compute song duration for file \"%1\"").arg(fileName), __FUNCTION__, __FILE__, __LINE__);
    ...
    if (res != FMOD_OK)
    {
        mediaManager->logError(tr("can't find the format for the file \"%1\"").arg(fileName), __FUNCTION__, __FILE__, __LINE__);
    }
    else
    {
        switch (type)
        {
            default:
                mediaManager->logError(tr("unknown format"), __FUNCTION__, __FILE__, __LINE__);
                delete song;
                return nullptr;
    ...
    connect(song, SIGNAL(songModified()), mediaManager, SLOT(onSongModified()));
...
QList<CSong *> CSong::loadAllSongsFromDatabase(CMediaManager * mediaManager)
{
    ...
    QSqlQuery query2(mediaManager->getDataBase());
    ...
    while (query.next())
    {
        ...
        if (!query2.exec())
        {
            mediaManager->logDatabaseError(query2.lastError().text(), query2.lastQuery(), __FILE__, __LINE__);
        }
        else
        {
            while (query2.next())
...
void CSong::startPlay()
{
    ...
    FMOD_RESULT res;
    res = m_mediaManager->getSoundSystem()->playSound(FMOD_CHANNEL_FREE, m_sound, true, &m_channel);
    ...
    if (m_cdRomDrive)
    {
        ...
        if (res != FMOD_OK)
            m_mediaManager->logError(tr("can't play track #%1 in CD-ROM drive \"%2\"").arg(m_cdRomTrackNumber + 1).arg(m_cdRomDrive->getDriveName()), __FUNCTION__, __FILE__, __LINE__);
    }

    if (res == FMOD_OK && m_channel)
    {
        res = m_channel->setMute(m_mediaManager->isMute());
        Q_ASSERT(res == FMOD_OK);

        res = m_channel->setVolume(static_cast<float>(m_mediaManager->getVolume()) / 100.0f);
        Q_ASSERT(res == FMOD_OK);
...
bool CSong::loadSound()
{
    ...
    // Chargement du son
    //res = m_mediaManager->getSoundSystem()->createStream(qPrintable(m_properties.fileName), FMOD_LOOP_OFF | FMOD_HARDWARE | FMOD_2D, nullptr, &m_sound);
    res = m_mediaManager->getSoundSystem()->createStream(reinterpret_cast<const char *>(m_properties.fileName.utf16()), FMOD_UNICODE | FMOD_LOOP_OFF | FMOD_HARDWARE | FMOD_2D, nullptr, &m_sound);
    ...
    if (res == FMOD_OK && m_sound)
    {
        ...
        if (res == FMOD_OK)
        {
            ...
            {
                m_mediaManager->logError(tr("duration of song \"%1\" has to be updated"), __FUNCTION__, __FILE__, __LINE__);
    ...
    m_mediaManager->logError(tr("the file \"%1\" can't be opened with FMOD").arg(m_properties.fileName), __FUNCTION__, __FILE__, __LINE__);
...
void CSong::updateDatabase()
{
    ...
    if (m_isModified && (m_properties != m_propertiesDB || m_infos != m_infosDB))
    {
        ...
        int artistId = m_mediaManager->getArtistId(m_infos.artistName, m_infos.artistNameSort);
        if (artistId < 0) artistId = 0;
        int albumId = m_mediaManager->getAlbumId(m_infos.albumTitle, m_infos.albumTitleSort);
        if (albumId < 0) albumId = 0;
        int albumArtistId = m_mediaManager->getArtistId(m_infos.albumArtist, m_infos.albumArtistSort);
        if (albumArtistId < 0) albumArtistId = 0;
        int genreId = m_mediaManager->getGenreId(m_infos.genre);
